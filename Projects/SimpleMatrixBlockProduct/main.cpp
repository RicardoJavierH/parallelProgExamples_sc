//Square matrix product by block decomposition, without using threads or Blas
//library
...
int main(){
    int n=6;//Order of square matrices A and B to computing the product
...
void simpleProduct(int n,double* ptrA,double* ptrB,double* ptrC){
    for (int i=0;i<n;i++)
        for (int j=0;j<n;j++){
            double aux=0;
            for (int k=0;k<n;k++)
                aux+=*(ptrA+i*n+k)*(*(ptrB+k*n+j));
            *(ptrC+i*n+j)=aux;//Cij=*(ptrC+i*n+j);
    }
};
//This function is for compute blocks of the product matrix (C11=A11B11+A12B21)
void blockCij (int k,double* A1,double* A2,double* B1,double* B2,double* Res){
